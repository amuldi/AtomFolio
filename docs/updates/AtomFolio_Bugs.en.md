...
| --- | --- | --- | --- | --- |
| (found during a production-readiness audit) `GET /api/health` on production (`atomfolio.vercel.app`) reports `portfolioStore.driver: "memory"` — no Postgres is connected, so portfolio data lives only in serverless instance memory | `DATABASE_URL` was never set in Vercel's production environment variables — `server/portfolioStore.mjs` is designed to silently fall back to the `memory` driver in that case, so the app kept working normally on the surface while running in a state where data can vanish on any cold start or redeploy | The fallback itself is left in place (it's needed), but `GET /api/health` now surfaces this loudly via a new `readiness` field (`server/productionReadiness.mjs`) — reported as `store-driver-not-durable` under `readiness.errors`. The actual fix (connecting a Neon `DATABASE_URL`) is an operational action, not a code change, so it isn't part of this commit — logged as the top item in `docs/production-readiness.md` | ⚠️ Detection only, connection still pending | (this session) |
