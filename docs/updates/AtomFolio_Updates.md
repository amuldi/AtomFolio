...
이 문서의 스크린샷은 전부 **실제로 로컬에서 띄운 웹 앱**을 캡처한 것이지만, 메뉴바 앱은
macOS 네이티브 트레이 위젯이라 이 세션이 가진 브라우저 자동화 도구로는 캡처할 수 없었습니다
(Electron 렌더러 HTML을 브라우저에 단독으로 띄워보려 시도했지만, `window.atomfolio`
IPC 브릿지가 없어 바로 오류가 나며 빈 화면만 나왔습니다 — 이 자체가 이 앱이 웹과 독립적으로
얼마나 깊게 네이티브 IPC에 의존하는지 보여주는 정황이기도 합니다). 직접 확인하려면
`desktop/README.md`의 안내대로 `cd desktop && npm install && npm run dev`로 실행해 보시면
됩니다.
