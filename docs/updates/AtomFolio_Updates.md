...
메뉴바 앱은 macOS 네이티브 트레이 위젯이라, 이 문서의 다른 스크린샷들처럼 브라우저 자동화로
그냥 캡처할 수는 없었습니다. `window.atomfolio`(Electron `preload.cjs`가 주입하는 IPC 브릿지)
없이 렌더러 HTML을 그냥 열면 즉시 오류가 나며 빈 화면만 나옵니다 — 이 자체가 이 앱이 얼마나
깊게 네이티브 IPC에 의존하는지 보여주는 정황이기도 합니다.

그래서 **실제 렌더러 코드(`atom-view.bundle.js`, `popover.js`)는 그대로 두고, `window.atomfolio`
자리에만 가짜 데이터를 반환하는 임시 스크립트를 하나 만들어 브라우저에 띄워봤습니다.** 즉
아래 스크린샷은 진짜 macOS 트레이 위젯을 캡처한 게 아니라, **이 프로젝트가 실제로 배포하는
렌더러 코드 그 자체를 목업 데이터로 구동한 화면**입니다 — 다르게 말하면 색상·폰트·레이아웃·
클릭 상호작용은 전부 진짜이고 숫자만 제가 지어낸 예시입니다. 이 목업용 임시 파일들
(`mock-bridge.js` 등)은 스크린샷을 찍은 뒤 바로 삭제했고 저장소에는 남아 있지 않습니다.
직접 진짜 트레이 앱을 확인하려면 `desktop/README.md` 안내대로
`cd desktop && npm install && npm run dev`로 실행해 보시면 됩니다.

**원자 위젯** — 보유 종목이 포트폴리오 총액을 중심으로 도는 궤도:

![메뉴바 원자 위젯 (렌더러 코드 그대로, 데이터는 목업)](assets/atomfolio-menubar-widget-mock.jpg)

**종목 클릭 시 상세 정보** — 평가금액·손익·비중이 표시됩니다:

![메뉴바 원자 위젯 — 종목 클릭 상세 (렌더러 코드 그대로, 데이터는 목업)](assets/atomfolio-menubar-widget-detail-mock.jpg)

**팝오버(뉴스/설정 페이저)** — 트레이 아이콘을 누르면 뜨는 패널:

![메뉴바 팝오버 (렌더러 코드 그대로, 데이터는 목업)](assets/atomfolio-menubar-popover-mock.jpg)
