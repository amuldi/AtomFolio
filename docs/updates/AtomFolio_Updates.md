...
> 합성하거나 상상해서 그린 스크린샷은 없습니다. 이전에는 "그때와 지금"을 한 번에 비교하는 단일
> 문서였는데, 지금부터는 작업 단위(커밋 묶음)마다 새 항목을 위에 추가하는 방식으로 바뀝니다 —
...
> 이 문서를 날짜별 changelog 구조로 바꾸기 전까지 있었던 단일 "그때와 지금" 비교 문서를 내용
> 그대로 옮긴 것입니다 — 다시 쓰지 않았습니다. README 원문 초기 커밋부터 이 항목을 작성한
> 시점까지의 누적 변화를 다룹니다.
...
**지금** (이 항목 작성 시점, 로컬 실행 캡처): 같은 "중심에서 뻗어나가는 구조"라는 컨셉은
