...
## 2026-08-16 — 자산군 자동 연동이 실제로는 안 먹히던 경우를 고침 (사용자 제보)

바로 아래 항목에서 자산군 자동 추론을 붙였는데, 사용자가 스크린샷과 함께 "아직도 직접해야하는데?"
라고 제보했습니다. 재현해보니 실제로 안 되는 경우가 있었습니다 — 자동 채움 로직이
`!current.trim() || current === '주식'`(지금 값이 비어있거나 기본값 '주식'일 때만 덮어씀) 조건으로
걸려 있었는데, 이건 "사용자가 일부러 고른 값"과 "이전에 다른 종목을 검색했을 때 자동으로 채워진
값이 그냥 남아있는 것"을 구분하지 못했습니다. 예를 들어 리츠 종목을 먼저 검색해서 자산군이
"리츠"로 자동 채워진 다음, 폼을 리셋하지 않고 곧바로 종목 티커에 채권 ETF 티커(TLT)를 새로
입력하면 — 새 조회 결과의 자산군("채권")이 있는데도 현재 값이 "리츠"(≠ '주식')라서 조건을 통과하지
못해 그대로 "리츠"에 멈춰 있었습니다. 사용자가 겪은 게 정확히 이 패턴이었을 가능성이 큽니다.

`current === '주식'`라는 문자열 비교 대신, "사용자가 이 조회에 대해 자산군을 직접 고른 적이
있는가"만 추적하는 `manualAssetClassTouchedRef`로 바꿨습니다. 종목명/티커를 다시 입력하면
`false`로 풀리고(새 조회니까 다시 추론해도 됨), 자산군 드롭다운을 사용자가 직접 바꾸면 그때만
`true`로 잠깁니다. 제안 목록에서 종목을 고르거나 "현재가 적용"을 누르는 것도 "이 종목으로
확정한다"는 의도가 명확한 동작이라 마찬가지로 다시 `false`로 풀어서, 그 시점부터의 자산군 값을
무조건 신뢰합니다. 기존 종목을 "수정"으로 열 때는 반대로 처음부터 `true`로 잠가서, 이미 저장된
자산군이 편집 중 우연히 다시 조회되는 시세 때문에 조용히 바뀌지 않게 했습니다.

로컬 dev 서버에서 실제 버그 재현 시나리오 그대로 재확인: "Realty Income" 선택 → 자산군 "리츠" →
폼을 그대로 둔 채 종목 티커에 "TLT" 직접 입력 → 이전에는 "리츠"에 멈춰 있던 게 이제 "채권"으로
자동 전환되는 것 확인.

**검증**: `npm run lint` 클린, `npm test` 126개 중 125 통과·1 skip(기존과 동일)·0 실패,
`npm run build` 클린. 위 재현 시나리오를 로컬 dev 서버 + 브라우저로 직접 다시 확인.
...
정작 스크린샷 5장은 하나도 다시 찍지 않은 채 커밋했습니다 — 사용자가 직접 확인하고 지적해서
발견했습니다. 실제로 브라우저로 배포된 사이트(`atomfolio.vercel.app`)에 접속해 명령 팔레트·
요약 도구·투자 시뮬레이션·시장 뉴스 4개 화면을 다시 캡처해 같은 파일명으로 교체했습니다.
메인 포트폴리오(원자 장면) 캡처만은 그대로 뒀는데, 이 세션에서 그 장면 자체의 시각적 렌더링을
바꾼 코드 변경이 없었고, 여러 포트폴리오가 쌓인 지금 계정에서는 카메라가 전체를 다 담으려고
축소되어 원래 캡처보다 오히려 원자가 작게 나와 더 나은 대체본이 되지 못했습니다 — 이 부분은
정직하게 예전 캡처를 남겨뒀다고 README 배너에 그대로 적었습니다.

다시 찍으면서 원래 캡처보다 오히려 더 나은 예시가 나온 것도 있습니다 — 명령 팔레트 캡처는
"TIGER"로 검색해 `TIGER 미국S&P500`(360750)이 `pension-growth`·`global-diversified`·
`sh 포트폴리오` 세 포트폴리오에 동시에 나타나는 걸 보여줘서, 본문이 말하는 "같은 종목이 여러
포트폴리오에 흩어져 있어도 어디 소속인지 함께 보여줍니다"를 원래 캡처보다 더 직접적으로
증명합니다. 시장 뉴스 캡처는 이번 세션에서 고친 KST 날짜 버그가 실제로 잘 작동하는 것도 같이
보여줍니다("08. 15. 오후 09:41"로 정확히 표시됨).
