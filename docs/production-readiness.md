...
### 데스크톱 앱 연동 (device token)

데스크톱 메뉴바 앱은 Electron이라 Clerk의 호스팅된 OAuth 리다이렉트 플로우를 그대로 쓸 수
없다. 그렇다고 계정 워크스페이스(`user:<id>`)를 게스트처럼 취급해 인증 없이 열어줄 수도
없다 — 그건 로그인 보호 자체를 무력화한다. 대신 `server/deviceTokens.mjs`가 별도의 "연결
코드" 발급 체계를 제공한다: 로그인한 사용자가 웹 설정에서 코드를 생성하면
(`atomfolio_dt_...`), 서버는 그 코드의 SHA-256 해시만 저장하고(원본은 저장하지 않음),
데스크톱 앱은 이 코드를 `Authorization: Bearer` 헤더로 보내 Clerk JWT와 동등하게 인증된다
(`workspaceAccess.mjs`의 `verifyClerkOrDeviceToken`). Postgres 전용 기능이다 — DB가 없으면
발급 자체가 명확한 에러로 실패한다(조용히 게스트로 폴백하지 않음).

**정책**: 사용자당 살아있는 코드는 항상 하나뿐이다 — 새로 발급하면 이전 코드는 즉시
무효화된다. 재발급은 반드시 Clerk 세션으로만 가능하다(기존 device token으로는 새 device
token을 못 만든다) — 코드가 유출됐을 때 실제로 유효한 복구 경로가 되도록 하기 위해서다.
아직 없는 것: 코드별 이름/기기 구분(현재는 전부 한 번에 해제만 가능), 코드 사용 이력 UI.
