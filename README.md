...
- **⌘-드래그로 이동**: 원자 위젯을 ⌘ 키를 누른 채 드래그하면 진짜 macOS 창 드래그로 옮겨집니다
  (이전엔 커서 위치를 흉내 내는 가짜 드래그였습니다) — 화면 가장자리에서 잠깐 버티면 macOS
  자체의 Mission Control 기능으로 다른 Space/Split View로 넘어갈 수 있는 것도 이 방식으로
  바꾼 이유입니다.
