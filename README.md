...
![메뉴바 요약 패널 — 실제 macOS 캡처](docs/updates/assets/atomfolio-menubar-summary-live.jpg)

- **팝오버**: 트레이 아이콘을 클릭하면 뜨는 패널로, 요약(포트폴리오 미니 대시보드 + 빠른
  종목 추가) · 종목 뉴스 검색 · 설정이 카드 스와이프로 전환되는 3페이지 페이저에 들어 있고,
  열 때마다 항상 요약 페이지로 돌아옵니다.
- **드래그로 이동**: 원자 위젯의 빈 배경(노드가 아닌 곳)을 드래그하면 진짜 macOS 창 드래그로
  옮겨집니다(이전엔 커서 위치를 흉내 내는 가짜 드래그였습니다) — 화면 가장자리에서 잠깐
  버티면 macOS 자체의 Mission Control 기능으로 다른 Desktop(Space)으로 넘어갈 수 있는 것도
  이 방식으로 바꾼 이유입니다(실제 마우스 제스처가 필요해 이 부분은 직접 확인이 필요합니다).
